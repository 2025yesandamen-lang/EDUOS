...
export default function App() {
  ...
  // Automated background sync for resilience actions without requiring manual user button clicks
  useEffect(() => {
    if (isEffectiveOnline && pendingActions.length > 0 && !isSyncingAll && token) {
      const autoSyncTimer = setTimeout(() => {
        handleSyncNow();
      }, 1500);
      return () => clearTimeout(autoSyncTimer);
    }
  }, [isEffectiveOnline, pendingActions.length, isSyncingAll, token]);
  ...
  const handleSyncNow = async () => {
    ...
    for (const action of pendingActions) {
      try {
        let attemptStr = localStorage.getItem(`cbt_active_attempt_${action.examId}`);
        let attemptId = "";
        if (attemptStr) {
          try {
        ...
        if (!attemptId) {
          attemptId = localStorage.getItem(`cbt_active_attempt_id_${action.examId}`) || "";
        }
        ...
        if (res.ok) {
          ...
          // If server rejects because attempt was already submitted, clear the stale unsynced key
          const errData = await res.json().catch(() => null);
          if (errData?.message?.includes("already submitted") || res.status === 400) {
            const key = `cbt_unsynced_${action.examId}`;
            const currentUnsynced = JSON.parse(localStorage.getItem(key) || "{}");
            delete currentUnsynced[action.qId];
            if (Object.keys(currentUnsynced).length === 0) {
        ...
        <>
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-6">
              ...
              {/* CBT Offline Sync Queue Interface - Visible ONLY to ADMIN */}
              {user.role === "ADMIN" && (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-[10px] font-mono uppercase">Administrative Resilience Registry</p>
                      </div>
                    </div>
